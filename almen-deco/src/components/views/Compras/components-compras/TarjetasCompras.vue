    ...
    <TransitionGroup class="row row-cols-1 row-cols-md-2 g-4 justify-content-center align-items-center" name="list" tag="ul">
      <div v-for="producto in productosFiltrados" :key="producto.id" class="card mx-auto" style="width: 18rem">
        ...
        <div class="card-body">
          ...
          <a href="#" class="btn btn-primary" @click.prevent="emitirInfoProducto(producto.id)">+ Info</a>
        </div>
      </div>
    </TransitionGroup>
  </div>
...
export default {
  ...
  methods: {
    ...
    emitirInfoProducto(productId) {
      this.$emit('producto-seleccionado', productId);
    },
  },
  watch: {
    category(newCategory) {
      this.aplicarFiltros(newCategory ? [newCategory] : []);
    }
  }
};
</script>
